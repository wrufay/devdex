...
export async function renderReview(screen, navigate) {
  ...
  const content = blessed.text({
    parent: box, top: 2, left: 2, right: 2, bottom: 3,
    tags: true, align: 'center', valign: 'middle',
  });

  const footer = blessed.text({
    parent: box, bottom: 1, left: 2, right: 2, tags: true, align: 'center',
  });
  ...
  if (cards.length === 0) {
    content.setContent('{green-fg}{bold}All caught up!{/bold}{/green-fg}\n\nNo cards are due for review.');
  ...
  function showFront() {
    ...
    footer.setContent('{gray-fg}Space/Enter = show answer  ·  Esc = back{/gray-fg}');
    screen.render();
  }
  ...
  function showBack() {
    ...
    footer.setContent(
      RATINGS.map((r) => `{${r.color}-fg}[${r.key}] ${r.label}{/${r.color}-fg}`).join('   ')
    );
    screen.render();
  }
  ...
  async function rate(quality) {
    ...
    if (index >= cards.length) {
      content.setContent('{green-fg}{bold}Session complete!{/bold}{/green-fg}\n\nYou reviewed every due card.');
      footer.setContent('{gray-fg}Esc = back to menu{/gray-fg}');
      screen.render();
      return;
    }
  ...
  box.key(['escape'], () => navigate('menu'));
  box.key(['space', 'enter'], () => { if (!showingBack) showBack(); });
  for (const r of RATINGS) {
    box.key([r.key], () => { if (showingBack) rate(r.quality); });
  }
